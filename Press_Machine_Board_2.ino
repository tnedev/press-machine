...
// Operation values; they show current states of operation and logic
...
void serialEvent3() 
/*
	Function to read the incoming communication data used with the software
	It is a good idea to change from Serial 3 to Serial 0 when all the testing is done
*/
{
	byte data; 
...
*/
{
	if(in_Nozhica_Nach1==true || in_Nozhica_Nach2==true) // Check if the cutters are in initial position
	{
		while(1)
		{
	...
	if(in_Razmotalka_NozhStart==false && in_Razmotalka_NozhStop==false) // Check if the roller cutter is in position
	{
		while(1)
		{
